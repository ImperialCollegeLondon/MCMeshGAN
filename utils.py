...
def save_meshdata(save_path, meshname, generated_vert, source_faces):
...
class AortaMesh_Dataset(data.Dataset):
    def __init__(self, data_lists, label_lists):
        super(AortaMesh_Dataset, self).__init__()
        self.data_paths = data_lists
        self.label_paths = label_lists

    def __getitem__(self, index):
        source_mesh_name, target_mesh_name, source_verts, source_verts_5000, source_verts_2500, source_verts_1250, target_vert, label, source_faces = load_mesh_data(self.data_paths, index)
        return source_mesh_name, target_mesh_name, source_verts, source_verts_5000, source_verts_2500, source_verts_1250, target_vert, label, source_faces
...
def load_mesh_data(data_path, index):
    # (1) find the name of the source data (.obj) using pytorch3D
    source_mesh_path = data_path[index]
    source_mesh_name = source_mesh_path.split('/')[-1]

    # (2) find the patient ID and select a different time point mesh as target mesh
    patient_ID = source_mesh_name.split('_')[0]
    target_candidate_meshes_path=[target_mesh_path for target_mesh_path in data_path if patient_ID in target_mesh_path and target_mesh_path!=source_mesh_path]
    target_mesh_path = target_candidate_meshes_path[random.randint(0, len(target_candidate_meshes_path)-1)]
    target_mesh_name = target_mesh_path.split('/')[-1]

    # (3) Compute the time interval, and prepare the label (time interval, age, gender)
    ...
    # (4) Load source and target mesh data (.obj) using pytorch3D
    # load multi-scale source_verts: 0: 10000, 1: 5000, 2: 2500, 3:1250
    source_verts_multiscale=[]
    # load 10000 verts
    source_verts, source_faces, source_aux = load_obj(source_mesh_path)
    # load 5000 verts
    source_mesh_path_5000=source_mesh_path.replace('10000', '5000')
    source_verts_5000, source_faces_5000, source_aux_5000 = load_obj(source_mesh_path_5000)
    # load 2500 verts
    source_mesh_path_2500=source_mesh_path.replace('10000', '2500')
    source_verts_2500, source_faces_2500, source_aux_2500 = load_obj(source_mesh_path_2500)
    # load 1250 verts
    source_mesh_path_1250=source_mesh_path.replace('10000', '1250')
    source_verts_1250, source_faces_1250, source_aux_1250 = load_obj(source_mesh_path_1250)

    # load target source mesh
    target_verts, target_faces, target_aux = load_obj(target_mesh_path)

    # (5) transform the pytorch3D edge format to PyTorch Geometric edge format, as our GNN is implemented in PyTorch Geometric
    ...
    return source_mesh_name, target_mesh_name, source_verts, source_verts_5000, source_verts_2500, source_verts_1250, target_verts, label, source_faces

def load_test_mesh_data(data_path, index):
    # (1) find the patient ID
    source_mesh_path = data_path[index]
    source_mesh_name = source_mesh_path.split('/')[-1]
    patient_ID = source_mesh_name.split('_')[0]
    candidate_meshes = [p for p in data_path if patient_ID in p]
    candidate_meshes.sort()

    # (2) Select source and target meshes
    # The first one as source, the last one as target here, or you can select all the rests as targets from the candidate meshes
    source_mesh_path = candidate_meshes[0]
    source_mesh_name = source_mesh_path.split('/')[-1]
    # The last or (rest) as target
    target_mesh_path = candidate_meshes[-1]
    target_mesh_name = target_mesh_path.split('/')[-1]
